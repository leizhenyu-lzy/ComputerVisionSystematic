...
class MyDifferenceOfGaussianPyramid():
    ...
    class MyOctave():
        ...
        def locateDiscreteKeyPointsInDOGs(self, wantedUsefulDOGs):
            # 先使用threshold筛选
            pointThreshold = 0.5*(self.T)/wantedUsefulDOGs
            pointsAfterThresholdInBool = (self.octaveDOGs)>pointThreshold  # 矩阵操作，返回一个相同形状的布尔值矩阵
            print("[locateDiscreteKeyPointsInDOGs] : No.%d DOGs 共有 %d 个点超过阈值 %f"%(self.octaveID, pointsAfterThresholdInBool.sum(), pointThreshold))
            # 找出离散的极值点
            ...
            for i in range(1,(self.octaveDOGs).shape[0]-1):  # sigma
                for j in range(1,(self.octaveDOGs).shape[1]-1):  # row
                    for k in range(1,(self.octaveDOGs).shape[2]-1):  # col
                        if pointsAfterThresholdInBool[i,j,k]==True:
                            maxValInCube = ((self.octaveDOGs)[i-1:i+1,j-1:j+1,k-1:k+1]).max()
                            minValInCube = ((self.octaveDOGs)[i-1:i+1,j-1:j+1,k-1:k+1]).min()
                            tempPointVal = (self.octaveDOGs)[i,j,k]
                            if (maxValInCube == tempPointVal) or (minValInCube == tempPointVal):  # 极大或极小都算
                    ...
                    for k in range(1,(self.octaveDOGs).shape[2]-1):
                        if discreteExtremaPointsInBool[i,j,k]==True:  # 在离散极值点附近搜索
                            ...
                            for iter in range(iterateMaxTimes):  # 限制迭代次数
                                ...
                                if (tempOffsetFloat>10000).sum() != 0:
                                ...
                                if (tempOffsetFloat.__abs__()<stopOnceOffsetLimit).sum() == 3:  # 相邻两次迭代的三个分量的偏移都在容许范围内，当前迭代点可能为极值(不一定是(i,j,k))
                                    if (totalOffset.__abs__()<totalOffsetUpperLimit).sum() == 3:  # 和最初的位置的偏移可以接受，符合泰勒展开在初始点附近近似的要求
                                        tempFunctionVal = (self.octaveDOGs)[i,j,k] + 0.5*(derivMatrix.T)*tempOffsetFloat
                                        hessianMatrixXY = hessianMatrix[1:,1:]  # 右下2×2为XY的矩阵
                                        trHessianXY = hessianMatrixXY.trace()
                                        detHessianXY = np.linalg.det(hessianMatrixXY)
                                        # 消除低对比度的点以及边缘效应
                                        if (abs(tempFunctionVal)>=(self.T/self.wantedUsefulDOGs)) and (((trHessianXY**2/detHessianXY)<12.1) and (detHessianXY>0)):
